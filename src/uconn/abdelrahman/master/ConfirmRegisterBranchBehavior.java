 ...
 * @author Abdelrahman
 */
public class ConfirmRegisterBranchBehavior extends CyclicBehaviour{
    ...
    public void action() {
        ...
        if(recommenderReply != null){
            if(recommenderReply.getPerformative() == ACLMessage.CONFIRM){
                System.out.println(myAgent.getName() + ":branch -> successfully submitted manifest");
                System.out.println(myAgent.getName() + ":branch -> recommender says: " + recommenderReply.getContent());
            }else {
                System.err.println(myAgent.getName() + ":branch -> recommender rejected the manifest submitted!");
